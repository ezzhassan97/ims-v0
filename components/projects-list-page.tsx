...
const PRIMARY_STATUSES: ProjPrimaryStatus[] = ["Launch", "On-Sale", "On-Hold", "Sold-Off"]
...
export function ProjectsPage({ rows: rowsProp, hideDeveloperFilter = false, embedded = false }: { rows?: ProjectRow[]; hideDeveloperFilter?: boolean; embedded?: boolean } = {}) {
  ...
  const applyPrimary = (r: ProjectRow, next: ProjPrimaryStatus, opts?: { excludedPhaseIds?: string[]; launchId?: string; startDate?: string; endDate?: string; launchTo?: "Closed" | "Inactive" }) => {
    ...
    // Cascaded phases leaving Launch: their active launches get the same
    // Closed/Inactive choice and end date as the main's.
    if (next !== "Launch") {
      rows.forEach((x) => {
        const hit = !r.isPhase && x.isPhase && x.mainProject?.id === r.id
        if (hit && !excluded.has(x.id) && x.primaryStatus === "Launch") {
          const act = launchesForProject(x.id).find((l) => l.launchStatus === "Active" && l.type === "Launch" && isIngestedLaunch(l))
          if (act) closeLaunch(act.id, opts?.endDate || undefined, opts?.launchTo)
        }
      })
    }
...
function LaunchRow({ l, radio, selected, onSelect, onView, topBorder, afterStatus }: {
  l: Launch
  radio?: boolean
  selected?: boolean
  onSelect?: () => void
  onView: () => void
  topBorder?: boolean
  /** Rendered right after the status tag — e.g. the → destination-status picker. */
    ...
    <Sheet open onOpenChange={(o) => { if (!o) onClose() }}>
      <SheetContent side="right" className="!w-[520px] !max-w-[95vw] p-0 flex flex-col">
        ...
        <div className="flex-1 space-y-4 overflow-y-auto px-5 py-4">
          ...
          {/* Property offerings */}
          <div className="rounded-lg border border-border">
            <p className="border-b border-border bg-muted/40 px-4 py-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Property Offerings ({launch.offerings.length})</p>
            <div className="divide-y divide-border">
              {launch.offerings.map((o) => (
                <div key={o.name} className="flex items-center justify-between px-4 py-2 text-sm">
                  <span className="font-medium">{o.name}</span>
                  <span className="text-xs text-muted-foreground">{o.propertyType} · {o.grossAreaRange} · {o.priceRange} EGP</span>
                </div>
              ))}
 ...
 *  - Moving OUT of Launch closes the currently Active launch/release; the end
 *    date is mandatory. Moving INTO Launch picks WHICH ingested linked launch
 *    to activate.
 *  - Phase cascade depends on from → to: On-Hold/Sold-Off destinations cascade
 *    (opt-out); On-Sale from On-Hold/Sold-Off recovers matching phases
 *    (opt-in, unselected by default — the Sold-Off recovery never touches
...
export function PrimaryStatusDialog({ r, phases, main, onClose, onConfirm }: {
  ...
  /** Every phase already On-Hold or Sold-Off — nothing under this main is selling. */
  const allPhasesClosed = phases.length > 0 && phases.every((p) => p.primaryStatus === "On-Hold" || p.primaryStatus === "Sold-Off")
  ...
  /** Per-row impacted counts — numbers emphasized. */
  const phaseCountsLine = (row: ProjectRow) => {
    const s = sums([row])
    const num = (n: number) => <span className="font-medium text-foreground">{n}</span>
    return (
      <>Properties Impacted: {num(s.launchAll)} Launch, {num(s.paG)} Primary Automatic, {num(s.pmG)} Primary Manual</>
    )
  }

  const viewProjectBtn = (p: ProjectRow) => (
    <span
      role="button" title="Open details in a new tab"
      onClick={(e) => { e.stopPropagation(); window.open(`/projects/${p.id}`, "_blank", "noopener,noreferrer") }}
      className="inline-flex h-7 w-7 shrink-0 cursor-pointer items-center justify-center rounded-md border border-border text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
    >
      <ExternalLink className="h-3.5 w-3.5" />
    </span>
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="flex flex-col gap-0 p-0 sm:max-w-3xl" style={{ maxHeight: "90vh" }}>
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {(["Launch", "On-Sale", "On-Hold", "Sold-Off"] as ProjPrimaryStatus[]).map((s) => (
              <button
                key={s} type="button" onClick={() => pickTarget(s)}
                className={cn(
            ...
            <div className="space-y-1.5">
              ...
              <div className="max-h-56 overflow-y-auto rounded-lg border border-border">
                {linkedLaunches.map((l, i) => (
                  <LaunchRow
                    key={l.id} l={l} radio topBorder={i > 0}
                    selected={selLaunchId === l.id}
                    onSelect={() => { setSelLaunchId(l.id); setStartDate(l.startDate ?? "") }}
                    onView={() => setDrawerLaunch(l)}
                  />
                ))}
              </div>
            </div>
        ...
        {leavingLaunch && activeLaunch && (
          <div className="space-y-1.5">
            ...
            <div className="rounded-lg border border-border">
              <LaunchRow
                l={activeLaunch}
                onView={() => setDrawerLaunch(activeLaunch)}
                afterStatus={
                  <>
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <button
                          type="button" title="Status the launch gets after this change"
                          className={cn("inline-flex shrink-0 cursor-pointer items-center gap-0.5 rounded-md border px-1.5 py-0.5 text-[10px] font-medium transition-opacity hover:opacity-80", LAUNCH_STATUS_TONE[launchTo])}
                        >
                          {launchTo}
                      ...
                      <DropdownMenuContent align="end" className="w-60">
                        {(["Closed", "Inactive"] as const).map((s) => (
                          <DropdownMenuItem key={s} onClick={() => setLaunchTo(s)} className="gap-2">
                            ...
                            <span className="text-xs text-muted-foreground">{s === "Closed" ? "End date required (default)" : "No end date needed"}</span>
                          </DropdownMenuItem>
                        ))}
                      </DropdownMenuContent>
                    </DropdownMenu>
        ...
        {/* Launch date fields, directly below the launch list/card */}
        {target === "Launch" && selLaunch && (
          <div className="space-y-1.5">
        ...
        {leavingLaunch && (
          <div className="space-y-1.5">
            ...
            {launchTo === "Closed" && (
              <p className="rounded-lg border border-blue-200 bg-blue-50 p-2.5 text-[11px] leading-4 text-blue-800">
                The dates when the launch was actively collecting EOIs — Sales Portal will be notified to flag EOIs outside this date range.
              </p>
            )}
          </div>
        )}

        {/* Hide the Available Primary properties (Launch destination) */}
        ...
        {target === "" ? (
          ...
          <div className="space-y-1 rounded-lg border border-border bg-muted/20 px-3 py-2.5">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
              What happens to the properties{selectable && cascading ? ` — main + ${includedPhases.length} phase${includedPhases.length !== 1 ? "s" : ""}` : ""}
            </p>
        ...
        {cascading && target !== "" && (selectable ? (
          eligiblePhases.length > 0 || phaseMode === "optout" ? (
            <div className="space-y-1.5">
              ...
              <div className="max-h-96 overflow-y-auto rounded-lg border border-border">
                {[r, ...eligiblePhases].map((p, i) => {
                  const isPhaseRow = p.id !== r.id
                  const isIncluded = !isPhaseRow || selectedPhases.has(p.id)
                  return (
                    <div key={p.id} className={cn("space-y-1 px-3 py-2.5", i > 0 && "border-t border-border/70", isPhaseRow && "bg-muted/20")}>
                      <div className="flex items-center gap-2.5">
                        {isPhaseRow && <Checkbox checked={isIncluded} onCheckedChange={() => togglePhase(p.id)} className="h-4 w-4 flex-shrink-0" />}
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-sm font-medium text-foreground">{p.name}</p>
                          <IdTag value={p.id} />
                        </div>
                        ...
                        <ArrowRight className="h-3.5 w-3.5 flex-shrink-0 text-muted-foreground" />
                        {isIncluded
                          ? <Tag value={target} cls={PRIMARY_COLORS[target as ProjPrimaryStatus]} />
                          : <span className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Unchanged</span>}
                        {viewProjectBtn(p)}
                      </div>
                      {isIncluded && phaseMode !== "optinSoldOff" && (
                        <div className={cn("text-[10px] text-muted-foreground", isPhaseRow && "pl-7")}>{phaseCountsLine(p)}</div>
                      )}
                    </div>
                  )
                })}
...
export function CascadeChangeDialog({ kind, targets, ignored, allRows, onClose, onConfirm }: {
  ...
  // Data may lack integrity: even an entity already at the destination type can hold
  // mis-flagged properties (hidden Availables of the destination type, shown ones of the
  // other). Rerunning fixes them — so every target and phase is always in scope.
  const includedEntryPhases = impacted.filter((p) => !excludedPhases.has(p.id))
  const entryChangingCount = targets.length + includedEntryPhases.length
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-2xl">
          ...
          <>
            ...
            {kind === "developer" && (
              <div className="space-y-1.5">
                ...
                <div className="max-h-96 overflow-y-auto rounded-lg border border-border">
                  {(() => {
                    // A fixed leading slot on EVERY row keeps titles aligned whether or not
                    // that row happens to have an opt-in checkbox.
                    const hasSelection = !destDevHidden && !!destDev && showable.length > 0
                    return [...targets, ...impacted].map((p, i) => {
        ...
        {/* Entry kind: the main project and EVERY phase, each with its own shown/hide impact —
            listed even when already at the destination type, since rerunning fixes mis-flagged
            properties. Phases stay excludable; the main isn't. */}
        {kind === "entry" && (
          <div className="space-y-1.5">
            <p className="text-xs text-muted-foreground">
              {impacted.length > 0
                ? <>Main project and its {impacted.length} phase{impacted.length > 1 ? "s" : ""} with their impacted properties — untick a phase to exclude it:</>
                : <>Impacted properties:</>}
            </p>
            <div className="max-h-96 overflow-y-auto rounded-lg border border-border">
              {[...targets, ...impacted].map((p, i) => {
                const excludable = impacted.includes(p)
                const isExcluded = excludable && excludedPhases.has(p.id)
                ...
                return (
                  <div key={p.id} className={cn("space-y-1.5 px-3 py-2.5", i > 0 && "border-t border-border/70", excludable && "bg-muted/20", isExcluded && "opacity-45")}>
                    <div className="flex items-center gap-2.5">
                      {excludable && <Checkbox checked={!isExcluded} onCheckedChange={() => toggleExcludedPhase(p.id)} className="h-4 w-4 flex-shrink-0" />}
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-medium text-foreground">{p.isPhase && targets.length > 1 ? `${p.mainProject?.name} — ${p.name}` : p.name}</p>
                        <IdTag value={p.id} />
                      </div>
