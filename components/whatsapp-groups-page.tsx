...
import { TableCard, TableCardHeader, TableToolbar, TableFooter, FilterSelect, DateRangeFilter, IdTag, COL_SEP } from "@/components/table-kit"
...
interface WaGroup {
  ...
  name: string
  image: string
  developer: { name: string; id: string; logo: string }
  members: number
  classified: number
  totalMedia: number
...
const WA_GROUPS: WaGroup[] = [
  ...
  { id: "375", jid: "120363268011461750@g.us", name: "Nawy - Dal", image: img("DAL", "15803d"), developer: { name: "DAL Developments", id: "375", logo: img("DA", "14532d") }, members: 409, classified: 130, totalMedia: 554, invitationLink: null, status: "Active", lastMessageSent: "2026-07-05T22:11:00", lastMediaSent: "2026-07-05T22:11:00", createdAt: "2024-03-20T17:37:00", updatedAt: "2026-07-05T22:11:00" },
  { id: "336", jid: "120363169719213706@g.us", name: "Nawy - Acasa Mia", image: img("ACM", "1f2937"), developer: { name: "Dar Al Alamia Developments", id: "336", logo: img("DA", "111827") }, members: 506, classified: 226, totalMedia: 887, invitationLink: null, status: "Active", lastMessageSent: "2026-07-05T22:06:00", lastMediaSent: "2026-07-05T22:06:00", createdAt: "2023-10-09T11:45:00", updatedAt: "2026-07-05T22:06:00" },
...
const DEVELOPER_OPTIONS = [...new Map(WA_GROUPS.map((g) => [g.developer.id, g.developer])).values()]
...
export function WhatsAppGroupsPage() {
  ...
  }, [q, developerF, statusF, dateFrom, dateTo])

  const pageRows = filtered.slice((page - 1) * pageSize, page * pageSize)

  if (selected) return <WhatsAppGroupDetails group={selected} onBack={() => setSelected(null)} />
    ...
    <div className="min-h-screen bg-secondary/40">
      <div className="space-y-4 p-6">
        ...
        <TableToolbar
          ...
          searchPlaceholder="Chat name"
          activeFilters={[developerF, statusF].filter(Boolean).length + ((dateFrom || dateTo) ? 1 : 0)}
          filters={
            <>
              ...
              <FilterSelect label="Status" value={statusF} options={["Active", "Inactive"]} onChange={(v) => { setStatusF(v); setPage(1) }} className="w-36" />
        ...
        <TableCard>
          <TableCardHeader title="WhatsApp groups" count={filtered.length} />
          <div className="overflow-x-auto">
            <table className={cn("w-max text-sm", COL_SEP)}>
              <thead className="border-b border-border bg-muted/60 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                <tr>
                  <Th className="pl-5">Group Name</Th>
                  <Th>Developer</Th>
                  <Th>Members #</Th>
                  <Th>Classified Media</Th>
                  <Th>Invitation Link</Th>
                  <Th>Last Message Sent</Th>
                  <Th>Last Media Sent</Th>
                  <Th>Created At</Th>
              ...
              <tbody className="divide-y divide-border">
                {pageRows.map((g) => (
                  <tr key={g.id} onClick={() => setSelected(g)} className="group cursor-pointer transition-colors hover:bg-muted/40">
                    <td className="py-3 pl-5 pr-4"><EntityCell image={g.image} name={g.name} id={g.jid} rounded="rounded-lg" /></td>
                    <td className="px-4 py-3"><EntityCell image={g.developer.logo} name={g.developer.name} id={g.developer.id} /></td>
                    <td className="px-4 py-3 tabular-nums">{g.members}</td>
                    <td className="px-4 py-3"><span className="font-semibold text-primary">{g.classified.toLocaleString()}</span> <span className="text-muted-foreground">/ {g.totalMedia.toLocaleString()}</span></td>
                    <td className="px-4 py-3 text-muted-foreground">{g.invitationLink ?? "-"}</td>
                    <td className="whitespace-nowrap px-4 py-3 text-xs text-muted-foreground">{fmtDateTime(g.lastMessageSent)}</td>
                    <td className="whitespace-nowrap px-4 py-3 text-xs text-muted-foreground">{fmtDateTime(g.lastMediaSent)}</td>
                    <td className="whitespace-nowrap px-4 py-3 text-xs text-muted-foreground">{fmtDateTime(g.createdAt)}</td>
                ...
                {filtered.length === 0 && <tr><td colSpan={9} className="px-5 py-16 text-center text-sm text-muted-foreground">No groups match your filters.</td></tr>}
              </tbody>
            </table>
          </div>
          <TableFooter page={page} pageSize={pageSize} total={filtered.length} onPage={setPage} onPageSize={(n) => { setPageSize(n); setPage(1) }} label="results" />
        </TableCard>
      </div>
    </div>
  )
