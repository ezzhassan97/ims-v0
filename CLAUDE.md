...
- **Selection pickers**: project dropdowns use the shared `ProjectTreeSelect` (nested phases, ids, statuses); area dropdowns use `AreaTreeSelect` (areas group their indented subareas, every row shows its ID, subareas caption their parent area — feed it `AREA_TREE`); developer dropdowns use `DeveloperSelect` (status tag + ID caption per row); dependent dropdowns (Category → Type → Subtype) reset children on parent change.
