...
- Vercel project: **`ims`** — project ID `prj_ELl78nAdrrPf0tQ69zGc0USsgfT1` (team `ezzhassan97-organization`). It owns the `ims-nawy.vercel.app` domain.
...
- **Selection pickers (MANDATORY on every page and form)**: project dropdowns use the shared `ProjectTreeSelect` (nested phases, ids, statuses); area dropdowns use `AreaTreeSelect` (areas group their indented subareas, every row shows its ID, subareas caption their parent area — feed it `AREA_TREE` or a page-built tree); developer dropdowns use `DeveloperSelect` (status tag + ID caption per row); dependent dropdowns (Category → Type → Subtype) reset children on parent change. All three support `multi` (`values`/`onValuesChange`) for toolbar/drawer filters — used on the properties pages (all sale types + the embedded Properties tab under project details) and in the property creation forms. Never hand-roll an area/developer/project dropdown; option rows are `<button>`s, so use the shared span-based `CheckBox`, never the shadcn `Checkbox`, inside them.
